"""FastAPI router for the Books app."""
...
_db = AppDatabase(Path(__file__).parent / "data" / "books.db")
_db.init()
...
def create_item(item: Dict[str, Any]):
    ...
    publish("books.item.created", {"id": item_id, **item})
    return created
...
def update_item(item_id: int, item: Dict[str, Any]):
    ...
    publish("books.item.updated", {"id": item_id, **item})
    # Publish specialised event when a book is marked completed
    if item.get("status") == "completed":
        publish("books.item.completed", {"id": item_id, **item})
    return updated
...
def delete_item(item_id: int):
    if not _db.delete_item(item_id):
        raise HTTPException(status_code=404, detail=f"Item {item_id} not found")
    publish("books.item.deleted", {"id": item_id})
    return {"success": True}
